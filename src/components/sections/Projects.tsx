          ...
          {filteredProjects.map((project, index) => (
            <motion.div
              ...
              className={`premium-card p-8 relative group hover:scale-105 transition-all duration-300 ${
                project.featured ? 'ring-2 ring-cyan-500 ring-offset-2' : ''
              }`}
            >
              {/* Project header */}
              <div className="flex items-start justify-between mb-6">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-sky-400 via-cyan-400 to-teal-400 text-white">
                      {getProjectCategory(project)}
                    </span>
                    {project.featured && (
                      <span className="px-3 py-1 rounded-full text-xs font-medium bg-cyan-100/50 text-cyan-700 border border-cyan-200/50">
                        Featured
                      </span>
                    )}
                  </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.techStack.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm font-medium hover:bg-cyan-100/50 hover:text-cyan-600 transition-colors"
                    >
                      {tech}
                    </span>
                  ))}
